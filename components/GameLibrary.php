<div class="bg-branddark flex flex-row gap-5 p-2 rounded-xl" id="game<?= $game->id ?>">
    ...
    <div class="ml-auto my-auto">
        <?php if (!is_null($game->getLatestBuild())) { ?>
            <a href="/library/<?= $game->id ?>/<?= urlencode(
    $game->getLatestBuild()->version,
) ?>" class="text-lg text-gray-200 bg-alt hover:bg-alt-600 rounded-xl p-2" target="_blank" rel="noopener noreferrer">Descargar</a>
        <?php } else { ?>

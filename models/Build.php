...
class Build
{
    ...
    public static function getByGame(Game|int $game)
    {
        ...
        $buildDB = Connection::customQuery(
            ORION_DB,
            "SELECT * FROM " .
                self::$table .
                " WHERE game_id = ? ORDER BY release_date DESC",
            [$game_id],
    ...
    {
        $game_id = $game instanceof Game ? $game->id : $game;
        $build = Connection::customQuery(
            ORION_DB,
            "SELECT * FROM " .
                self::$table .
                " WHERE game_id = ? ORDER BY release_date DESC LIMIT 1",
            [$game_id],
        )->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function save(): bool
    {
        ...
        if (
            !isset($this->release_date) ||
            !self::get($this->game_id, $this->version)
        ) {
    ...
    public function setFile($file)
    {
        $uuid = $this->getUUID();
        $this->file = $uuid;
        return S3Helper::upload(
            EBUCKET_LOCATION::GAME_BUILD,
            $uuid,
            null,
            $file["type"],
            $file["tmp_name"],
    ...
    public function getFile()
    {
        return is_null($this->file)
            ? null
            : S3Helper::retrieve(EBUCKET_LOCATION::GAME_BUILD, $this->file);
    }
}

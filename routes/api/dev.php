...
$router->mount('/dev', function() use ($router) {
    ...
    $router->post('/achievement', function(){
        ...
        $iconPath = Tript::encryptString("orionach_".strval(count($game->getAchievements())));
        $lockedIconPath = null;

        S3Helper::upload(EBUCKET_LOCATION::GAME_ACHIEVEMENT, $iconPath, null, $icon['type'], $icon['tmp_name']);
        if ($lockedIcon) {
            $lockedIconPath = Tript::encryptString("orionach_".strval(count($game->getAchievements()))."_lock");
            S3Helper::upload(EBUCKET_LOCATION::GAME_ACHIEVEMENT, $lockedIconPath, null, $lockedIcon['type'], $lockedIcon['tmp_name']);
        }

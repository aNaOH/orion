...
function showPage()
{
    ...
    <div class="container mx-auto text-center">
        ...
        <p class="text-lg mt-4 flex flex-row gap-x-3 justify-center">
            <span>Escrita por <a href="/profile/<?= $post->getAuthor()
                ->id ?>" class="font-semibold hover:text-gray-300 link-underline"><?= $post->getAuthor()
    ->username ?></a></span>
            <gradient-chip
                base-color="<?= $gType->tint ?>"
                size="15"
                icon-path="https://cdn.orion.moonnastd.com/guidetype/<?= $gType->icon ?>"
                text="<?= $gType->type ?>"
                border-radius="8">
            </gradient-chip>
        </p>

...
$title = "Editar logro " . $achievement->name . " | Orion Dev Panel";
...
<div id="edit-achievement" class="tab-pane block">
  <form id="editAchievementForm" class="space-y-6">
    <?php OrionComponents::TokenInput(ETOKEN_TYPE::DEVACTION, [
        "userID" => $_SESSION["user"]["id"],
        "gameID" => $game->id,
    ]); ?>
    <input type="hidden" name="game" value="<?= $game->id ?>">
    <input type="hidden" name="achievement" value="<?= $achievement->id ?>">
    ...
    <div>
      ...
      >
        <option <?php if ($achievement->type == EACHIEVEMENT_TYPE::TRIGGERED) {
            echo "selected";
        } ?> value="0">Por activación</option>
        <option <?php if ($achievement->type == EACHIEVEMENT_TYPE::STAT) {
            echo "selected";
        } ?> value="1">Por estadística</option>
      </select>
    ...
    <div id="statContainer" class="hidden">
      ...
      >
        ...
        <?php foreach ($stats as $stat): ?>
          <option value="<?= $stat->id ?>"<?= $stat->id == $achievement->stat_id
    ? " selected"
    : "" ?>><?= $stat->name ?></option>
        <?php endforeach; ?>
      </select>
    ...
    <div>
        <div class="flex items-center gap-2">
          <input
            ...
            name="secret"
            type="checkbox"
            <?php if ($achievement->secret): ?>checked<?php endif; ?>
    ...
    <div class="flex flex-col gap-3 mt-6">
      ...
      >
        ...
        <span>Actualizar logro</span>
            ...
            <script src="/assets/js/forms/dev/achievement-edit.js"></script>
